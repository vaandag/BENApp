...
try {
  ...
  if ($path === 'users/profile' && $method === 'POST') {
    $uid=ben_require_user_id($pdo); $username=trim((string)($input['username']??'')); $bio=trim((string)($input['bio']??'')); $avatar=trim((string)($input['avatar_url']??'')); $bioFont=trim((string)($input['bio_font']??'default')); $bioColor=(int)($input['bio_color']??12307199);
  ...
  if (preg_match('#^users/(\d+)$#',$path,$m) && $method==='GET') {
    ...
    $viewer=ben_optional_user_id($pdo); $f=$pdo->prepare('SELECT 1 FROM follows WHERE follower_id=? AND following_id=?'); $f->execute([$viewer,(int)$m[1]]); $u['following_me']=(bool)$f->fetchColumn();
  ...
  if (preg_match('#^memories/(\d+)$#',$path,$m) && $method==='DELETE') {
    ...
    $memoryId=(int)$m[1];
    $exists=$pdo->prepare('SELECT id FROM memories WHERE id=? AND user_id=?');
    $exists->execute([$memoryId,$uid]);
    if(!$exists->fetchColumn()){ben_error('Anı bulunamadı veya silme yetkin yok.',404,'MEMORY_NOT_FOUND');}
    $pdo->beginTransaction();
    try {
      $pdo->prepare('DELETE FROM memory_likes WHERE memory_id=?')->execute([$memoryId]);
      $pdo->prepare('DELETE FROM memory_saves WHERE memory_id=?')->execute([$memoryId]);
      $pdo->prepare('DELETE FROM comments WHERE memory_id=?')->execute([$memoryId]);
      $pdo->prepare('DELETE FROM memories WHERE id=? AND user_id=?')->execute([$memoryId,$uid]);
      $pdo->commit();
      ben_ok();
    } catch(Throwable $e) {
      if($pdo->inTransaction())$pdo->rollBack();
      ben_error('Anı silinemedi.',500,'MEMORY_DELETE_FAILED');
    }
  }
  if (preg_match('#^memories/(\d+)/(like|save)$#',$path,$m) && $method==='POST') {
    $memoryId=(int)$m[1]; $userId=ben_require_user_id($pdo); $table=$m[2]==='like'?'memory_likes':'memory_saves';
  ...
  if ($path === 'messages' && $method === 'GET') { $uid=ben_require_user_id($pdo);$other=(int)($_GET['with_user_id']??0);if($other>0){$s=$pdo->prepare('SELECT * FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) ORDER BY id');$s->execute([$uid,$other,$other,$uid]);}else{$s=$pdo->prepare('SELECT * FROM messages WHERE sender_id=? OR receiver_id=? ORDER BY id');$s->execute([$uid,$uid]);}echo json_encode($s->fetchAll());exit; }
  if ($path === 'messages' && $method === 'POST') { $uid=ben_require_user_id($pdo); $receiver=(int)($input['receiver_id']??0); $body=trim((string)($input['body']??'')); if($receiver<=0||$body===''){ben_error('Geçerli alıcı ve mesaj gerekli.',422,'INVALID_MESSAGE');} $s=$pdo->prepare('INSERT INTO messages(sender_id,receiver_id,body) VALUES(?,?,?)'); $s->execute([$uid,(int)$input['receiver_id'],(string)$input['body']]); echo json_encode(['ok'=>true,'id'=>$pdo->lastInsertId()]); exit; }
  ...
  if ($path === 'follow' && $method === 'POST') {
    $from=ben_require_user_id($pdo); $to=(int)($input['following_id']??0);
  ...
  if ($path === 'notifications/read' && $method === 'POST') { $uid=ben_require_user_id($pdo); $pdo->prepare('UPDATE notifications SET is_read=1 WHERE user_id=?')->execute([$uid]); echo json_encode(['ok'=>true]); exit; }
  if ($path === 'notifications' && $method === 'GET') { $uid=ben_require_user_id($pdo); $s=$pdo->prepare('SELECT * FROM notifications WHERE user_id=? ORDER BY id DESC'); $s->execute([$uid]); echo json_encode($s->fetchAll()); exit; }
  ...
  if (ben_env('BEN_DEBUG','0') === '1') ben_error($e->getMessage(),500,'INTERNAL_ERROR');
  ben_error('Beklenmeyen bir sunucu hatası oluştu.',500,'INTERNAL_ERROR');
}
