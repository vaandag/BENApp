...
try {
  ...
  if ($path === 'users' && $method === 'GET') {
    $q=trim((string)($_GET['q']??'')); $s=$pdo->prepare('SELECT id,username,email,xp,coins,created_at FROM users WHERE username LIKE ? ORDER BY username LIMIT 30'); $s->execute(['%'.$q.'%']); echo json_encode($s->fetchAll()); exit;
  }
  ...
  if ($path === 'memories' && $method === 'GET') {
    ...
    $sql = "SELECT m.*, u.username AS username, (SELECT COUNT(*) FROM memory_likes l WHERE l.memory_id=m.id) AS likes_count, (SELECT COUNT(*) FROM memory_saves s WHERE s.memory_id=m.id) AS saves_count, (SELECT COUNT(*) FROM comments c WHERE c.memory_id=m.id) AS comments_count FROM memories m LEFT JOIN users u ON u.id=m.user_id WHERE (m.expires_at IS NULL OR m.expires_at > CURRENT_TIMESTAMP) AND (m.privacy='public' OR m.user_id=$uid OR (m.privacy='followers' AND EXISTS (SELECT 1 FROM follows f WHERE f.follower_id=$uid AND f.following_id=m.user_id))) ORDER BY m.id DESC";
  ...
  if (preg_match('#^memories/(\d+)/comments$#',$path,$m) && $method==='GET') {
    $s=$pdo->prepare('SELECT c.*, u.username FROM comments c LEFT JOIN users u ON u.id=c.user_id WHERE c.memory_id=? ORDER BY c.id ASC'); $s->execute([(int)$m[1]]); echo json_encode($s->fetchAll()); exit;
  }
  ...
  if ($path === 'messages' && $method === 'GET') { $s=$pdo->prepare('SELECT * FROM messages WHERE sender_id=? OR receiver_id=? ORDER BY id'); $uid=(int)($_GET['user_id']??1); $s->execute([$uid,$uid]); echo json_encode($s->fetchAll()); exit; }
  ...
  if ($path === 'messages/read' && $method === 'POST') { $s=$pdo->prepare('UPDATE messages SET is_read=1 WHERE receiver_id=?'); $s->execute([(int)($input['user_id']??1)]); echo json_encode(['ok'=>true]); exit; }
